...
O instalador Windows pode ser gerado localmente com `npm run desktop:build`. Para publicar uma versao no GitHub, atualize a versao em `package.json`, envie as alteracoes e crie/envie uma tag correspondente, por exemplo `v1.0.0`. A GitHub Action gera o instalador e o anexa a uma Release. Tambem e possivel executar a Action manualmente em **Actions > Build Windows > Run workflow** para baixar o instalador como artefato.
